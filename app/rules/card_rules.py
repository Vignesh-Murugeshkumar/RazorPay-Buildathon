...
def evaluate_visa_ce30(
    ...
    ip_matches = all(normalize_str(tx.ip_address) == curr_ip and curr_ip != "" for tx in orders_to_check)
...
def evaluate_mastercard_fpt(
    ...
    2. 3-tier matching matrix:
       ...
       - Tier 2 (Delivery Factor): Carrier proof of physical delivery or download logs.
    ...
    if not tier2_delivery:
        gaps.append("Mastercard FPT Tier 2 failed: Missing verified carrier delivery confirmation")
    if not tier3_auth:
...
def calculate_confidence_score(
    ...
    Sc = w_CE30 * m_CE30 + w_carrier * m_carrier + w_mfa * m_mfa + gps_bonus
    - w_CE30 = 55.0
    - w_carrier = 35.0
    - gps_bonus = 10.0 (if verified_gps)
    - w_mfa = 5.0 (if mfa_authenticated)
    
    Max score is 100.0.
    Gatekeeper rule: Sc >= 85.0 -> AUTO_DISPATCH, Sc < 85.0 -> ROUTE_TO_HITL_QUEUE
    """
    w_ce30 = 55.0 if network_compliant else 0.0
    w_carrier = 35.0 if carrier_delivered else 0.0
    w_gps = 10.0 if (carrier_delivered and carrier_gps_verified) else 0.0
    w_mfa = 5.0 if mfa_verified else 0.0
    
    raw_score = w_ce30 + w_carrier + w_gps + w_mfa
    score = min(100.0, round(raw_score, 2))
    
    breakdown = {
        "network_compliance_points": w_ce30,
        "carrier_delivery_points": w_carrier,
        "carrier_gps_bonus": w_gps,
...
def evaluate_dispute_compliance(payload: DisputePayload) -> RuleEvaluationResult:
    ...
    Routes to Visa CE 3.0 or Mastercard FPT based on card network.
    """
